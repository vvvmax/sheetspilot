 ...
 *
 */
	class SheetsPilotUniteSettings{
	    ...
	    public function __construct(){
	    	...
	    	$suffix = time()."_".SheetsPilotFunctions::getRandomString(5)."_";
		...
		private function modifySettingsData($arrSettings){
			...
			foreach($arrSettings as $key=>$content){
				switch(getType($content)){
					...
					case "object":
					case "array":
						$content = SheetsPilotFunctions::convertStdClassToArray($content);
					break;					
				}
		...
		public function getArrSettingNamesAndTitles(){
			...
			foreach($this->arrSettings as $setting){
				$name = SheetsPilotFunctions::getVal($setting, "name");
				$title = SheetsPilotFunctions::getVal($setting, "text");
				if(!empty($name))
					$arrNames[$name] = $title;
			}
		...
		public function getSap($sapKey){
			...
			$arrSap = SheetsPilotFunctions::getVal($this->arrSaps, $sapKey);
			if(empty($arrSap))
				SheetsPilotFunctions::throwError("sap with key: $sapKey not found");
		...
		private function getIndexByName($name){
			...
			if(!empty($this->arrIndex)){
				if(array_key_exists($name, $this->arrIndex) == false)
					SheetsPilotFunctions::throwError("setting $name not found");
			...
			foreach($this->arrSettings as $index=>$setting){
				$settingName = SheetsPilotFunctions::getVal($setting, "name");
				if($settingName == $name)
					return($index);
			}
			
			SheetsPilotFunctions::throwError("Setting with name: $name don't exists");
		}
		...
		public function getArrSettingsBySap($sapName){
			...
			if($sapKey === null)
				SheetsPilotFunctions::throwError("sap: $sapName not found");
		...
		public function getArrTextFromAllSettings(){
			...
			foreach($this->arrSettings as $setting){
				...
				$desc = SheetsPilotFunctions::getVal($setting, "description");
				...
				$items = SheetsPilotFunctions::getVal($setting, "items");
				if(!empty($items)){
		...
		public function getSettingValue($name,$default=""){
			...
			$value = SheetsPilotFunctions::getVal($setting, "value",$default);
		...
		public function addHr($name="",$params=array()){
			...
			if($name != "")
			  ...
			  $this->HRIdCounter++;
			  $itemName = "hr_".SheetsPilotFunctions::getRandomString();
		...
		public function addSap($text, $name="", $opened = false, $icon="", $params = array()){
			...
			if(empty($text))
				SheetsPilotFunctions::throwError("sap $name must have a text");
		...
		protected function addSettingGlobalParams($setting){
			...
			$settingType = SheetsPilotFunctions::getVal($setting, "type");
			foreach($this->arrGlobalParams as $param){
				
				$types = SheetsPilotFunctions::getVal($param, "types");
		...
		protected function add($name,$defaultValue = "",$text = "",$type = self::TYPE_TEXT,$arrParams = array()){
			...
			$modifyType = SheetsPilotFunctions::getVal($setting, "modifytype");
			if(!empty($modifyType)){				
		...
		public function addSettingByArray($setting){
			...
			if(is_array($setting) == false)
				SheetsPilotFunctions::throwError("addSettingByArray: Empty setting given");
			
			$name = SheetsPilotFunctions::getVal($setting, "name");
			
			SheetsPilotFunctions::validateNotEmpty($name, "setting name");
		...
		public function addControl($control_item_name,$controlled_item_name,$control_type,$value){
			...
			SheetsPilotFunctions::validateNotEmpty($control_item_name,"control parent");
			SheetsPilotFunctions::validateNotEmpty($controlled_item_name,"control child");
			SheetsPilotFunctions::validateNotEmpty($control_type,"control type");
		...
		public function setPairedSettings(){
			...
			foreach($this->arrSettings as $setting){
				$addSettingName = SheetsPilotFunctions::getVal($setting, self::PARAM_ADDFIELD);
		...
		public function loadXMLFile($filepath, $loadedSettingsType = null){
			...
			if(!file_exists($filepath))
				SheetsPilotFunctions::throwError("File: '$filepath' not exists!!!");
			
			if(function_exists("simplexml_load_file") == false)
				SheetsPilotFunctions::throwError("Your php missing SimpleXML Extension. The plugin can't work without this extension because it has many xml files to load. Please enable this extension in php.ini");
			
			$obj = simplexml_load_file($filepath);
			
			if(empty($obj))
				SheetsPilotFunctions::throwError("Wrong xml file format: $filepath");
			...
			foreach($fieldsets as $fieldset){
				...
				$sapName = (string)SheetsPilotFunctions::getVal($attribs, "name");
				$sapLabel = (string)SheetsPilotFunctions::getVal($attribs, "label");
				$sapIcon = (string)SheetsPilotFunctions::getVal($attribs, "icon");				
				$loadFrom = (string)SheetsPilotFunctions::getVal($attribs, "loadfrom");	
				$loadParam = (string)SheetsPilotFunctions::getVal($attribs, "loadparam");	
				$loadedSettingsType = (string)SheetsPilotFunctions::getVal($attribs, "loadtype");	
				$nodraw = (string)SheetsPilotFunctions::getVal($attribs, "nodraw");				
				$visibility = (string)SheetsPilotFunctions::getVal($attribs, "visibility");				
				...
				*/
				...
				$sapParams = array();
				if(!empty($nodraw)){
					$sapParams["nodraw"] = SheetsPilotFunctions::strToBool($nodraw);
				}
				
				SheetsPilotFunctions::validateNotEmpty($sapName,"sapName");
				...
				SheetsPilotFunctions::validateNotEmpty($sapLabel,"sapLabel");
				...
				$fields = SheetsPilotFunctions::getVal($fieldset, "field");
				...
				foreach($fields as $field){
					...
					$fieldType = (string)SheetsPilotFunctions::getVal($attribs, "type");
					$fieldName = (string)SheetsPilotFunctions::getVal($attribs, "name");
					$fieldLabel = (string)SheetsPilotFunctions::getVal($attribs, "label");
					$fieldDefaultValue = (string)SheetsPilotFunctions::getVal($attribs, "default");
					...
					SheetsPilotFunctions::validateNotEmpty($fieldType,"type");
					
					//validate name
					if($fieldType != self::TYPE_HR && $fieldType != self::TYPE_CONTROL &&
						$fieldType != "bulk_control_start" && $fieldType != "bulk_control_end")
						SheetsPilotFunctions::validateNotEmpty($fieldName,"name");		
					switch ($fieldType){
						case self::TYPE_CHECKBOX:
							
							$fieldDefaultValue = SheetsPilotFunctions::strToBool($fieldDefaultValue);
						...
						case self::TYPE_CONTROL:
							$parent = SheetsPilotFunctions::getVal($arrParams, "parent");
							$child =  SheetsPilotFunctions::getVal($arrParams, "child");
							$ctype =  SheetsPilotFunctions::getVal($arrParams, "ctype");
							$value =  SheetsPilotFunctions::getVal($arrParams, "value");
						...
						break;	
						default:
							SheetsPilotFunctions::throwError("wrong type: $fieldType");
						break;						
					}
		...
		public function addFromXmlFile($filepathXML){
			...
			$objNewSettings = new SheetsPilotUniteSettings();
		...
		private function getOptionsFromXMLField($field, $fieldName){
			...
			$options = SheetsPilotFunctions::getVal($arrField, "option");
			...
			foreach($options as $option){
				...
				if(gettype($option) == "string")
					SheetsPilotFunctions::throwError("Wrong options type: ".$option." in field: $fieldName");
				
				$attribs = $option->attributes();
				
				$optionValue = (string)SheetsPilotFunctions::getVal($attribs, "value");							
				$optionText = (string)SheetsPilotFunctions::getVal($attribs, "text");
				
				//validate options:
				SheetsPilotFunctions::validateNotEmpty($optionText,"option text");
		...
		public function mergeSettings($settings){
			...
			foreach($arrSettingsNew as $setting){
				...
				if(array_key_exists($sapOld, $arrNewSapKeys) == false)
					SheetsPilotFunctions::throwError("sap {$sapOld} should be exists in sap keys array");
				...
				if(array_key_exists($name, $this->arrIndex))
					SheetsPilotFunctions::throwError("The setting <b>{$name} </b> already exists. ");
		...
		private function addExternalSettings($filename, $loadParam = null, $loadType = null){
			...
			$filepathSettings = SheetsPilotGlobals::$pathSettings."{$filename}.xml";
			
			if(file_exists($filepathSettings) == false)
				SheetsPilotFunctions::throwError("The file: {$filename}.xml don't found in settings folder");
		...
		public function updateArrSettingByName($name,$setting){
			...
			foreach($this->arrSettings as $key => $settingExisting){
				$settingName = SheetsPilotFunctions::getVal($settingExisting,"name");
				if($settingName == $name){
		...
		public function modifyValueByDatatypeFunc($value, $datatype){
			...
			switch($datatype){
				...
				case self::DATATYPE_LINK:
					...
					$link = SheetsPilotFunctions::getHrefFromHtml($value);
					if(!empty($link))
						$value = $link;
		...
		public function setStoredValues($arrValues){
			...
			foreach($this->arrSettings as $key=>$setting){
				...
				$datatype = SheetsPilotFunctions::getVal($setting, "datatype");
								
				//skip custom type
				$customType = SheetsPilotFunctions::getVal($setting, "custom_type");
			...
			$this->sanitizeSelectStoredValues();
		
			return($arrValues);
		}

		/**
		 * Ensure select controls never keep a value missing from their option list (broken HTML select).
		 */
		private function sanitizeSelectStoredValues() {

			foreach ( $this->arrSettings as $key => $setting ) {

				$custom_type = SheetsPilotFunctions::getVal( $setting, 'custom_type' );
				...
				$items = SheetsPilotFunctions::getVal( $setting, 'items', array() );
				if ( empty( $items ) ) {
					continue;
				}

				$allowed = array_values( $items );
				$value   = SheetsPilotFunctions::getVal( $this->arrSettings[ $key ], 'value' );

				if ( in_array( $value, $allowed, true ) ) {
					continue;
				}

				$fallback = SheetsPilotFunctions::getVal( $this->arrSettings[ $key ], 'default_value' );
				if ( ! in_array( $fallback, $allowed, true ) ) {
					$fallback = $allowed[0];
				}

				$this->arrSettings[ $key ]['value'] = $fallback;
			}
		}
