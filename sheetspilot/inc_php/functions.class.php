...
if(!defined("SHEETSPILOT_INC")) die("restricted access");
	...
	class SheetsPilotFunctions{
		...
		/**
		 * Verify the SheetsPilot nonce sent with the current POST or GET request.
		 *
		 * @return bool
		 */
		private static function isSheetspilotRequestNonceValid() {
			$request_nonce = self::readFilteredInput( 'nonce', INPUT_POST );

			if ( null === $request_nonce || false === $request_nonce ) {
				$request_nonce = self::readFilteredInput( 'nonce', INPUT_GET );
			}
			...
			$request_nonce = sanitize_text_field( (string) $request_nonce );

			if ( '' === $request_nonce ) {
				return false;
			}

			return (bool) wp_verify_nonce( $request_nonce, 'sheetspilot_actions' );
		}

		/**
		 * Read a request variable via filter_input, with superglobal fallback.
		 *
		 * filter_input() often returns null on CGI/FastCGI and XAMPP even when $_POST/$_GET
		 * are populated. Fall back to superglobals so AJAX keeps working.
		 *
		 * @param string $name        Request parameter name.
		 * @param int    $input_type  INPUT_POST or INPUT_GET.
		 * @return mixed String or array value, or null when not present.
		 */
		private static function readFilteredInput( $name, $input_type ) {

			$value = filter_input( $input_type, $name, FILTER_DEFAULT );

			if ( null !== $value && false !== $value ) {
				return $value;
			}

			// filter_input() fallback (XAMPP/CGI). Nonce verified in isSheetspilotRequestNonceValid() or caller; values sanitized via sanitizeVar().
	
			if ( INPUT_POST === $input_type && isset( $_POST[ $name ] ) ) {
				return wp_unslash( $_POST[ $name ] );
			}

			if ( INPUT_GET === $input_type && isset( $_GET[ $name ] ) ) {
		...
		/**
		 * Read and sanitize a filtered request value.
		 *
		 * @param string $name          Request parameter name.
		 * @param int    $input_type    INPUT_POST or INPUT_GET.
		 * @param string $sanitize_type Sanitize type constant.
		 * @return mixed Sanitized value, or null when not present/invalid.
		 */
		private static function readSanitizedFilteredInput( $name, $input_type, $sanitize_type ) {
			$raw = self::readFilteredInput( $name, $input_type );

			if ( null === $raw || false === $raw ) {
				return null;
			}

			return self::sanitizeVar( $raw, $sanitize_type );
		}

		/**
		 * Whether the current user may read admin debug query flags without a plugin nonce.
		 *
		 * @return bool
		 */
		private static function currentUserCanReadDebugFlags() {
			...
			$capability = class_exists( 'SheetsPilotGlobals', false ) ? SheetsPilotGlobals::$capability : 'manage_options';

			return current_user_can( $capability );
		}

		/**
		 * Debug flags that may be read without a SheetsPilot nonce (admins only).
		 *
		 * @param string $name Request parameter name.
		 * @return bool
		 */
		private static function isDebugRequestFlagName( $name ) {
			return in_array( $name, array( 'showdebug', 'showtrace' ), true );
		}
		...
		public static function getPostGetVariable( $name, $initVar = '', $sanitizeType = '' ) {
			...
			if ( ! self::isSheetspilotRequestNonceValid() ) {
				if ( self::isDebugRequestFlagName( $name ) && self::currentUserCanReadDebugFlags() ) {
					$raw = self::readFilteredInput( $name, INPUT_GET );
					if ( null === $raw || false === $raw ) {
						$raw = self::readFilteredInput( $name, INPUT_POST );
					}
					if ( null !== $raw && false !== $raw ) {
						return self::sanitizeVar( $raw, $sanitizeType );
					}
				}
			...
			$post_value = self::readSanitizedFilteredInput( $name, INPUT_POST, $sanitizeType );
			if ( null !== $post_value ) {
				return $post_value;
			}

			$get_value = self::readSanitizedFilteredInput( $name, INPUT_GET, $sanitizeType );
			if ( null !== $get_value ) {
				return $get_value;
			}
		...
		public static function getGetVar( $name, $initVar = '', $sanitizeType = '' ) {
			...
			// Admin menu URLs use GET routing params without a plugin nonce.
			if ( is_admin() && in_array( $name, array( 'page', 'view' ), true ) ) {
				$get_value = self::readSanitizedFilteredInput( $name, INPUT_GET, $sanitizeType );
				if ( null === $get_value ) {
					return $initVar;
				}
			...
			// Admin debug flags (front or wp-admin) without a plugin nonce.
			if ( self::isDebugRequestFlagName( $name ) && self::currentUserCanReadDebugFlags() ) {
				$get_value = self::readSanitizedFilteredInput( $name, INPUT_GET, $sanitizeType );
				if ( null === $get_value ) {
					return $initVar;
		...
		/**
		 * Normalize a spreadsheet column / field key for cell rules (Unicode-safe).
		 * WordPress sanitize_key() strips non-ASCII letters; field names may use Hebrew or other scripts.
		 *
		 * @param string $key Raw key from editor or AJAX.
		...
		public static function normalize_cell_rule_field_key( $key ) {
			...
			$max_len = 500;
			if ( function_exists( 'mb_strlen' ) && function_exists( 'mb_substr' ) && mb_strlen( $key, 'UTF-8' ) > $max_len ) {
				$key = mb_substr( $key, 0, $max_len, 'UTF-8' );
			} elseif ( strlen( $key ) > $max_len ) {
				$key = substr( $key, 0, $max_len );
			}
			$result = $key;
			return $result;
		}
		...
		public static function getVal($arr,$key,$altVal=""){
			...
			if (!isset($arr[$key])) {
        		return self::sanitize($altVal);
			}

			return self::sanitize($arr[$key]);
		}
		...
		public static function convertStdClassToArray($d){
			...
			if (is_array($d)){
				...
				return array_map(array("SheetsPilotFunctions","convertStdClassToArray"), $d);
			} else {
				return $d;
			}
		...
		public static function getDiffArrItems($arr, $arrDefault, $arrMustKeys = array()){
			...
			if(!empty($arrMustKeys))
				$arrMustKeys = SheetsPilotFunctions::arrayToAssoc($arrMustKeys);
		...
		public static function assocToArrayKeyValue($assoc, $keyName, $valueName, $firstItem = null){
			...
			foreach($assoc as $item){
				if(!array_key_exists($keyName, $item))
					SheetsPilotFunctions::throwError("field: $keyName not found in array");
				
				if(!array_key_exists($valueName, $item))
					SheetsPilotFunctions::throwError("field: $valueName not found in array");
		...
		public static function trimArrayItems($arr){
			if(gettype($arr) != "array")
				SheetsPilotFunctions::throwError("trimArrayItems error: The type must be array");
		...
		public static function insertToAssocArray($arrItems, $arrNewItems){
			...
			foreach($arrNewItems as $item){
				$insertAfter = SheetsPilotFunctions::getVal($item, "insert_after");
		...
		/**
		 * Debug helper: print full array/object safely in HTML.
		 */
		public static function dmpArrayFull($data){
	
			$formatted = print_r($data, true);
			$html = '<div align="left" style="direction:ltr;max-width:100%;overflow:auto;">'
				. '<pre style="white-space:pre-wrap;word-break:break-word;">' . esc_html($formatted) . '</pre>'
				. '</div>';
			echo wp_kses(
				$html,
				array(
		...
		public static function arrayToXML($array, $rootName, $xml = null){
			...
			if($xml === null){
				...
				if($strXML === false)
					SheetsPilotFunctions::throwError("Wrong xml output");
		...
		public static function getArrSanitizeTypes(){
			...
			$arrSanitize = array();
			$arrSanitize[self::SANITIZE_ID] = __("Sanitize ID", "sheetspilot");
			$arrSanitize[self::SANITIZE_KEY] = __("Sanitize KEY", "sheetspilot");
			$arrSanitize[self::SANITIZE_TEXT_FIELD] = __("Sanitize Text Field", "sheetspilot");
			$arrSanitize[self::SANITIZE_NOTHING] = __("No Sanitize (not recomended)", "sheetspilot");
		...
		public static function getUrlContents($url){
			...
			$options = array("timeout"=>SheetsPilotGlobals::PASTE_OPERATION_TIMEOUT);
			...
			if($code == 403){		//forbidden
				...
				$urlBase = SheetsPilotFunctions::getBaseUrl($url);
		...
		public static function getBaseUrl($url){
			...
			$scheme = SheetsPilotFunctions::getVal($arrUrl, "scheme","http");
			$host = SheetsPilotFunctions::getVal($arrUrl, "host");
			$path = SheetsPilotFunctions::getVal($arrUrl, "path");
		...
		public static function validateObjectMethod($object, $strMethod, $objectName){
			...
			if(method_exists($object, "initByID") == false)
				SheetsPilotFunctions::throwError("Object: $objectName don't have method $strMethod");
		...
		public static function validateEmail($email, $fieldName="email"){
			...
			$code = str_replace( '%s', $fieldName, __("The %s is not valid", "sheetspilot") );
		...
		public static function mkdirValidate($path, $dirName){
			...
			if(is_dir($path) == false){
				@mkdir($path);
				if(!is_dir($path))
					SheetsPilotFunctions::throwError("$dirName path: {$path} could not be created. Please check your permissions");
			}
		...
		public static function writeFile($str, $filepath){
			...
			if(is_array($str))
				SheetsPilotFunctions::throwError("write file should accept only string in file: ". $filepath);
		...
		/**
		 * Write debug variables to a file (var_export per entry).
		 *
		 * @param string $filepath Target file path.
		 * @param array  $vars     Variables to dump (string keys are written as section labels).
		 * @return bool True on success, false if the file could not be opened.
		 */
		public static function writeDebugFile( $filepath, $vars = array(), $writeType = 'w' ) {
		...
		public static function clearDirByTime($path, $olderThenSeconds = 240,$isDebug = false){
			...
			if($isDebug){
				...
				$strTime = SheetsPilotFunctions::timestamp2DateTime($currentTime);
			...
			foreach($arrPaths as $file){
				...
				if($isDebug == true){
					...
					$strFiletime = SheetsPilotFunctions::timestamp2DateTime($filetime);
		...
		public static function downloadFile($filepath, $filename = null){
			...
			SheetsPilotFunctions::validateFilepath($filepath,"export file");
		...
		public static function toString( $value ) {
			...
			$output = json_encode( $value, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES );
			if ( ! is_string( $output ) ) {
				$output = '';
			}
