...
function UniteSettingsSheetsPilot(){
	...
	var g_objProvider = new UniteProviderAdminSheetsPilot();
	...
	if(!g_doublyAdmin)
		var g_doublyAdmin = new UniteAdminSheetsPilot();
	...
	function repeaterAddItem(event, objWrapper, objItemValues, objItemInsertAfter){
		...
		var objItemSettings = new UniteSettingsSheetsPilot();
	...
	/**
	 * init CodeMirror on textareas with class ubai-codemirror (sync value back to textarea for form save).
	 */
	function initCodeMirrorEditors() {
		...
		var $scope = g_objWrapper || g_objParent;
		if (!$scope || !$scope.length) {
			return;
		}

		$scope.find("textarea.ubai-codemirror").each(function () {
