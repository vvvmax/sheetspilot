...
if(!defined("SHEETSPILOT_INC")) die("restricted access");
	...
	class SheetsPilot_PluginDB{
		...
		public function __construct(){
			...
			$this->pdb = new SheetsPilot_PluginProviderDB();
		...
		private function throwError($message,$code=-1){
			SheetsPilotFunctions::throwError($message,$code);
		}
		...
		public function delete($table,$where){
			...
			SheetsPilotFunctions::validateNotEmpty($table,"table name");
			SheetsPilotFunctions::validateNotEmpty($where,"where");
		...
		public function getWhereCatIDWithAll($catID){
			...
			if($catID === "all"){
				...
				if(is_array($catID) == false)
					SheetsPilotFunctions::throwError("catIDs could be array or number");
		...
		public function update($tableName,$arrData,$where){
			...
			SheetsPilotFunctions::validateNotEmpty($tableName,"table cannot be empty");
			SheetsPilotFunctions::validateNotEmpty($where,"where cannot be empty");
			SheetsPilotFunctions::validateNotEmpty($arrData,"data cannot be empty");
		...
		public function fetchSql($query, $supressErrors = false){
			...
			$rows = SheetsPilotFunctions::convertStdClassToArray($rows);
		...
		public function updateRecordsOrdering($table, $arrIDs){
			...
			//get items assoc
			$arrRecords = SheetsPilotFunctions::arrayToAssoc($arrRecords,"id");
			
			$order = 0;
			foreach($arrIDs as $recordID){
				$order++;
				
				$arrRecord = SheetsPilotFunctions::getVal($arrRecords, $recordID);
		...
		public function fetchPage($tableName, $pagingOptions, $where="", $orderField="", $groupByField="", $sqlAddon=""){
			...
			$page = SheetsPilotFunctions::getVal($pagingOptions, "page");
			$rowsInPage = SheetsPilotFunctions::getVal($pagingOptions, "inpage");
			
			
			//valdiate and sanitize
			SheetsPilotFunctions::validateNumeric($page);
			SheetsPilotFunctions::validateNumeric($rowsInPage);
			SheetsPilotFunctions::validateNotEmpty($rowsInPage);
			if($page < 1)
				$page = 1;
		...
		public function fetchSingle($tableName,$where="",$orderField="",$groupByField="",$sqlAddon=""){
			...
			if(is_array($tableName)){
				...
				$tableName = SheetsPilotFunctions::getVal($arguments, "tableName");
				$where = SheetsPilotFunctions::getVal($arguments, "where");
				$orderField = SheetsPilotFunctions::getVal($arguments, "orderField");
				$groupByField = SheetsPilotFunctions::getVal($arguments, "groupByField");
				$sqlAddon = SheetsPilotFunctions::getVal($arguments, "sqlAddon");
				$errorEmpty = SheetsPilotFunctions::getVal($arguments, "errorEmpty");
			}
			
			if(empty($errorEmpty)){
				$tableTitle = SheetsPilotFunctions::getVal(self::$arrTableTitles, $tableName, __("Record", "sheetspilot"));
				
				$errorEmpty = $tableTitle." ".__("not found", "sheetspilot");
			}
